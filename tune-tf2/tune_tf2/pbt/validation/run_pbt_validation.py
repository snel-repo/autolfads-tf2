...
CFG_NAME = 'maze.yaml' # the default configuration of the LFADS model
DATA_GROUP = 'maze_230_trials' # the group ot datasets
NUM_WORKERS = 20 # should be 34 # the number of workers to use
# the hyperparameter space to search
HYPERPARAM_SPACE = {
    'TRAIN.LR.INIT': HyperParam(1e-5, 5e-3, explore_wt=0.3, 
        enforce_limits=True, init=0.004),
    'MODEL.DROPOUT_RATE': HyperParam(0.4, 1.0, explore_wt=0.3, # normally 0.0, 0.6
        enforce_limits=True, sample_fn='uniform'),
    'MODEL.CD_RATE': HyperParam(0.3, 0.99, explore_wt=0.3, # normally 0.01, 0.7
...
cfg_path = path.join(
    '/snel/home/asedler/core/tune_tf2/config', 
    CFG_NAME)
# the directory to store all of the PBT runs
PBT_HOME = path.join(
    '/snel/home/asedler/ray_results/pbt_validation_reconcileDropoutAndClipping', 
    DATA_GROUP)
# the directory from which to load the PBT datasets
DATA_HOME = path.join(
    '/snel/share/share/derived/maze_data/spikes_rates_velocity_2ms', 
    DATA_GROUP)
