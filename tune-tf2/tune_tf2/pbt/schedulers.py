import os, logging, json, shutil, copy, time
import numpy as np
...
from tune_tf2.defaults import PBT_CSV, EXPLOIT_CSV, HPS_CSV
...
class MultiStrategyPBT(FIFOScheduler):
    ...
    def on_trial_result(self, trial_runner, trial, result):
        ...
        get_gen_state = lambda t: self._trial_state[t][self._generation]
        ...
        if self._percent_improvement <= self._min_percent_improvement \
    ...
    def _log_exploit(self, old_state, new_state, old_trial, new_trial):
        ...
        log_path = os.path.join(
            old_trial.local_dir, EXPLOIT_CSV)
        exploit_data = {
            'generation': self._generation,
    ...
    def debug_string(self):
        ...
        pbt_state = "Generation {}/{}, {} Perturbs, {:.2E}% Improvement".format(
            self._generation, self._max_generations, 
            self._num_perturbations, self._percent_improvement)
