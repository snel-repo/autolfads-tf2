...
from lfads_tf2.defaults import DEFAULT_CONFIG_DIR, get_cfg_defaults
from lfads_tf2.utils import read_hps, read_fitlog, flatten, unflatten, restrict_gpu_usage
...
class RandomSearch:
    def __init__(self, search_path):
        ...
        if not path.exists(self.search_path):
            logger.info(
                f'Creating a new `RandomSearch` at {self.search_path}')
            self.complete = False
        else:
    ...
    def run(self, 
        ...
        self.model_dirs = self.get_model_dirs()
        self.search_hps = [h for h, r in cfg_samples.items() \
            if type(r) is tune.sample.sample_from]
        self.complete = True
    ...
    def retry_failed_runs(self, available_gpus=[0], models_per_gpu=2):
        ...
        for model_dir in failed_runs:
            failed_model_dir = path.join(
                path.dirname(model_dir), 'failed_model')
    ...
    def retry_failed_sample(self, available_gpus=[0] , models_per_gpu=2):
        ...
        if len(failed_runs) == 0:
            logger.info(
                f'Found no missing posterior samples at {self.search_path}')
            return
        # Give the user the opportunity to inspect the rerun directories
