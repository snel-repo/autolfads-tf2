...
def load_data(data_dir, 
    ...
    for h5_filename in h5_filenames:
        ...
        def reformat_inds(train_inds, valid_inds):
            # Corrects dimensionality and 1-indexing for MATLAB datasets
            train_inds = train_inds.squeeze()
            valid_inds = valid_inds.squeeze()
            min_ix = np.concatenate([train_inds, valid_inds]).min()
            train_inds -= min_ix
            valid_inds -= min_ix
        ...
        if signal == 'truth':
            ...
            # reformat indices in case they came from MATLAB
            train_data, valid_data = reformat_inds(train_data, valid_data)

        if merge_tv:
            # merge training and validation data
            if 'train_inds' in h5dict and 'valid_inds' in h5dict:
                # if there are index labels, use them to reassemble full data
                train_inds, valid_inds = reformat_inds(
                    h5dict['train_inds'], h5dict['valid_inds'])
...
def load_posterior_averages(model_dir,
    ...
    if merge_tv:
        ...
        sampling_output = (SamplingOutput(**train_output),
